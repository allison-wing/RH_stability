...
%load data
load(['RH_stab_' num2str(z1) '-' num2str(z2) '.mat'])
...
%%295%%
[CAPE_theory295,RH_theory295] = calculate_CAPE_theory(T_LCL295,T_LNB295,p_LCL295,eps_imp295small,PE_imp295small);
%%300%%
[CAPE_theory300,RH_theory300] = calculate_CAPE_theory(T_LCL300,T_LNB300,p_LCL300,eps_imp300small,PE_imp300small);
%%305%%
[CAPE_theory305,RH_theory305] = calculate_CAPE_theory(T_LCL305,T_LNB305,p_LCL305,eps_imp305small,PE_imp305small);

%Calculate change in simulated and theoretical CAPE with warming
dCAPEdT = (100./dT).*(cape305small-cape295small)./cape300small; %as %/K with respect to 10K delta SST
...
[CAPE_theory300,dC] = calculate_CAPE_derivatives(T_LCL300,p_LCL300,T_LNB300,eps_imp300small,PE_imp300small);
%     dCAPEdTLCL = dC(1,:);
%     dCAPEdpLCL = dC(2,:);
%     dCAPEdTLNB = dC(3,:);
%     dCAPEdeps = dC(4,:);
...
%Calculation decomposition, as %/K
...
%residual
dCAPE_resid = dCAPEdT - dCAPE_TLCL - dCAPE_pLCL - dCAPE_TLNB - dCAPE_eps - dCAPE_PE;
...
figure('position',[100 100 1400 500]); %for 1 x2
%%%
...
hold on
%plot box plot
h=boxplot([dCAPEdT(iPAR)' dCAPE_TLCL(iPAR)' dCAPE_pLCL(iPAR)' dCAPE_TLNB(iPAR)' dCAPE_eps(iPAR)' dCAPE_PE(iPAR)' dCAPE_resid(iPAR)'],'Labels',{'Full','T_LCL','p_LCL','T_LNB','Entrain.','PE','Resid.'},'Symbol','');
set(h,{'linew'},{2})
...
for i = 1:length(iPAR)
    ...
    plot([7],dCAPE_resid(iPAR(i)),'.','MarkerSize',30,'Color',prof300small(iPAR(i)).color)
end
%plot mean
...
plot([7.125],nanmean(dCAPE_resid(iPAR)),'k*','MarkerSize',12,'LineWidth',2)
...
hold on
%plot box plot
h=boxplot([dCAPEdT(iEXP)' dCAPE_TLCL(iEXP)' dCAPE_pLCL(iEXP)' dCAPE_TLNB(iEXP)' dCAPE_eps(iEXP)' dCAPE_PE(iEXP)' dCAPE_resid(iEXP)'],'Labels',{'Full','T_LCL','p_LCL','T_LNB','Entrain.','PE','Resid.'},'Symbol','');
set(h,{'linew'},{2})
%plot individual models
for i = 1:length(iEXP)
    ...
    plot([7],dCAPE_resid(iEXP(i)),'.','MarkerSize',30,'Color',prof300small(iEXP(i)).color)
end
%plot mean
...
plot([7.125],nanmean(dCAPE_resid(iEXP)),'k*','MarkerSize',12,'LineWidth',2)
...
gcfsavepdf('Fig10-dCAPE-decompose.pdf')
...
hold on
%plot box plot
h=boxplot([dCAPEdT(iLES-2)' dCAPE_TLCL(iLES-2)' dCAPE_pLCL(iLES-2)' dCAPE_TLNB(iLES-2)' dCAPE_eps(iLES-2)' dCAPE_PE(iLES-2)' dCAPE_resid(iLES-2)'],'Labels',{'Full','T_LCL','p_LCL','T_LNB','Entrain.','PE','Resid.'},'Symbol','');
set(h,{'linew'},{2})
%plot individual models
for i = 1:length(iLES-2)
    ...
    plot([7],dCAPE_resid(iLES(i)-2),'.','MarkerSize',30,'Color',prof300small(iLES(i)-2).color)
end
%plot mean
...
plot([7.125],nanmean(dCAPE_resid(iLES-2)),'k*','MarkerSize',12,'LineWidth',2)
...
hold on
%plot box plot
h=boxplot([dCAPEdT(iLES-1)' dCAPE_TLCL(iLES-1)' dCAPE_pLCL(iLES-1)' dCAPE_TLNB(iLES-1)' dCAPE_eps(iLES-1)' dCAPE_PE(iLES-1)' dCAPE_resid(iLES-1)'],'Labels',{'Full','T_LCL','p_LCL','T_LNB','Entrain.','PE','Resid.'},'Symbol','');
set(h,{'linew'},{2})
%plot individual models
for i = 1:length(iLES-1)
    ...
    plot([7],dCAPE_resid(iLES(i)-1),'.','MarkerSize',30,'Color',prof300small(iLES(i)-1).color)
end
%plot mean
...
plot([7.125],nanmean(dCAPE_resid(iLES-1)),'k*','MarkerSize',12,'LineWidth',2)
...
hold on
%plot box plot
h=boxplot([dCAPEdT(iLES)' dCAPE_TLCL(iLES)' dCAPE_pLCL(iLES)' dCAPE_TLNB(iLES)' dCAPE_eps(iLES)' dCAPE_PE(iLES)' dCAPE_resid(iLES)'],'Labels',{'Full','T_LCL','p_LCL','T_LNB','Entrain.','PE','Resid.'},'Symbol','');
set(h,{'linew'},{2})
%plot individual models
for i = 1:length(iLES)
    ...
    plot([7],dCAPE_resid(iLES(i)),'.','MarkerSize',30,'Color',prof300small(iLES(i)).color)
end
%plot mean
...
plot([7.125],nanmean(dCAPE_resid(iLES)),'k*','MarkerSize',12,'LineWidth',2)
