...
switch lower(c.type)
    ...
    case 'sam'
     ...
     case 'fms'
       ...
       e0 = 610.78; 
       esl = c.e0 * exp(c.Lv0/c.Rv*(1.0/c.T0 - 1.0./T));
       esi = esl;

     case 'fmsi'
       % Simple thermodynamics with ice
      
       e0 = 610.78; 
       esl = c.e0 * exp(c.Lv0/c.Rv*(1.0/c.T0 - 1.0./T));
       esi = c.e0 * exp(c.Ls0/c.Rv*(1.0/c.T0 - 1.0./T));

         
         
     otherwise
    
       error('Unknown thermodynamics type. Cannot calculate saturation vapor pressure')
