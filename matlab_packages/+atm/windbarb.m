function [] = windbarb(lon,lat,u,v,varargin)
%WINDBARBM Plot wind barbs
%
%  Based on Nick Siler's windbarbm, which plotted windbarbs onto a map axes
%
%  WINDBARB(lat,lon,u,v) projects two dimensional wind barbs onto the 
%  current axes. The vector components (u,v) are in units of knots and
%  are specified at the points (lat,lon). It handles winds up to 130 knots.
%  Winds exceeding 130 knots will appear as 130 knots.
%
%  WINDBARB(lat,lon,u,v,s) uses the input s to scale the vectors after 
%  they have been automatically scaled to fit within the grid. If omitted, 
%  s = 0.9 is assumed.
%  
%  WINDBARB(lat,lon,u,v,'PropertyName',PropertyValue,...) and
%  WINDBARB(lat,lon,u,v,s,'PropertyName',PropertyValue,...) uses the 
